...
function hometicket(content) {
    ...
    const text = content.text.split("NEZBYTNÉ OHRANIČENÍ VSTUPENKY (MUSÍ BÝT I POD VSTUPENKOU !)").slice(1)
    const reservations = []
    ...
    for (let i = 0; i < text.length; i++) {
        ...
        let code = codeRegex.exec(page)
        let datePlacePrice = priceRegex.exec(page)

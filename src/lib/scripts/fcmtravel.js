...
const terminalRegex = /.*Terminal\s(.*)/;
...
function main(text) {
    ...
    while (true) {
        ...
        // departure terminal is optional
        const depTerminal = depLine[4].match(terminalRegex);
        if (depTerminal) {
            res.reservationFor.departureTerminal = depTerminal[1];
        }
        ...
        // arrival terminal is optional
        const arrTerminal = arrLine[4].match(terminalRegex);
        if (arrTerminal) {
            res.reservationFor.arrivalTerminal = arrTerminal[1];
        }
